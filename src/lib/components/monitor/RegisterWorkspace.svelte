<script lang="ts">
	...
	let registers = $derived(view.registerMap?.registers);
	let watchlistKey = $derived(view.watchlist.join(','));
	let registersByAddress = $derived(indexRegisters(registers ?? []));
	let staleAddresses = $derived(new Set(view.missingAddresses));
	let watchedAddresses = $derived(new Set(watchlistKey ? watchlistKey.split(',').map(Number) : []));
	...
	<CardContent class="p-4">
		<Tabs bind:value={activeTab}>
			<TabsList variant="line" class="mb-2">
				...
				>
				<TabsTrigger value="uart">Transport Log</TabsTrigger>
			</TabsList>
			...
			<TabsContent value="all" class="min-w-0">
				...
				{#if view.missingAddresses.length > 0 && !view.polling}
					...
					>
						{view.missingAddresses.length} stale values retained from the previous scan.
					</div>
				{/if}
				{#if !view.snapshot}
					<div
						class="flex min-h-32 items-center justify-center gap-2 rounded-lg border border-dashed bg-muted/40 p-6 text-muted-foreground"
					>
						...
						{#each view.watchlist as address (address)}
							...
							<div class="group relative">
								<button
									type="button"
									class={[
										'flex w-full items-center justify-between rounded-lg border bg-muted/50 p-3 pr-10 text-left transition-colors hover:border-accent-foreground/40 hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring focus-visible:outline-none',
